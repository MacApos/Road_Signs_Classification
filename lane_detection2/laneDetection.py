...
right_fitx = right_fit[0] * ploty ** 2 + right_fit[1] * ploty + right_fit[2]
    ...
    # return left_fitx, right_fitx, plotly
...
if (len(leftx) == 0 or len(rightx) == 0) or (len(righty) == 0 or len(lefty) == 0):
    ...
    print(len(nonzerox[left_lane_inds]), len(nonzeroy[left_lane_inds]),
          len(nonzerox[right_lane_inds]), len(nonzeroy[right_lane_inds]))

    # left_fitx, right_fitx, plotly = fit_poly(img_shape, leftx, lefty, rightx, righty)
    ...
    # print(out_img, left_curverad, right_curverad)
