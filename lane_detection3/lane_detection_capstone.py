...
def scale_and_perspective(image, left_curve, right_curve, src, dst, shape, perspective=False):
    width = image.shape[1]
    height = image.shape[0]
...
def visualise_perspective(image, left_curve, right_curve, src, dst, shape):
...
def detect_lines(path):
    ...
    for path in data_list:
        ...
        t_leftx0, t_lefty0, t_rightx0, t_righty0 = scale_and_perspective(image, left_curve0, right_curve0,
                                                                         src, dst, image.shape, perspective=False)
        t_left_curve0, t_right_curve0 = fit_poly(t_leftx0, t_lefty0, t_rightx0, t_righty0)

        curves0 = np.concatenate((left_curve0, right_curve0))
        t_curves0 = np.concatenate((t_left_curve0, t_right_curve0))
        big.append(curves0)
        big_warp.append(t_curves0)

        leftx, lefty, rightx, righty = scale_and_perspective(image, left_curve0, right_curve0,
                                                             src, dst, original.shape, perspective=False)
        t_leftx, t_lefty, t_rightx, t_righty = scale_and_perspective(image, left_curve0, right_curve0,
                                                                     src, dst, original.shape, perspective=True)
        ...
        small.append(curves)
        small_warp.append(t_curves)

        poly, frame = visualise_perspective(frame, left_curve0, right_curve0, src, dst, original.shape)

        if not frame_exists and not label_exists:
            cv2.imwrite(save_frame, frame)
            cv2.imwrite(save_label, poly)
            print(i, path, 'saving frame and label')

        elif not frame_exists and label_exists:
            cv2.imwrite(save_label, poly)
            print(i, path, 'saving frame')

        elif frame_exists and not label_exists:
            cv2.imwrite(save_label, poly)
            print(i, path, 'saving label')

        else:
            print(i, path, 'already processed')

        i += 1

    pickle.dump(small, open(f'MLND-Capstone/small_labels.p', 'wb'))
    pickle.dump(small_warp, open(f'MLND-Capstone/small_warp_labels.p', 'wb'))
    pickle.dump(big, open(f'MLND-Capstone/big_labels.p', 'wb'))
    pickle.dump(big_warp, open(f'MLND-Capstone/big_warp.p', 'wb'))

path = r'C:\Nowy folder\10\Praca\Datasets\MLND-Capstone'
